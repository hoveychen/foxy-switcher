...
// PickProviderForDevice picks from one provider pool for one device. Every
// provider is exclusive: accounts another device holds are filtered out, and
// the pick falls back to idle-reclaim when that empties the pool.
