...
func (s *Store) ComputeAttribution(ctx context.Context, accountID int64) (AccountAttribution, error) {
	...
	fhAcc, fhUn := attributeWindow(rows, segs, now, func(r UsageHistoryRow) float64 { return r.FiveHourUtil })
	sdAcc, sdUn := attributeWindow(rows, segs, now, func(r UsageHistoryRow) float64 { return r.SevenDayUtil })
	ssAcc, ssUn := attributeWindow(rows, segs, now, func(r UsageHistoryRow) float64 { return r.SevenDaySonnetUtil })
...
func attributeWindow(rows []UsageHistoryRow, segs []LeaseEvent, now int64, util func(UsageHistoryRow) float64) (map[string]float64, float64) {
	...
	for i := 1; i < len(rows); i++ {
		...
		if d := util(cur) - util(prev); d < 0 {
			// Window rollover — start the current window fresh.
			accum = map[string]float64{}
			unattr = 0
		...
		distribute(contribution, prev.Timestamp, cur.Timestamp, segs, now, accum, &unattr)
	}
	return accum, unattr
}
...
func distribute(amount float64, from, to int64, segs []LeaseEvent, now int64, accum map[string]float64, unattr *float64) {
	if to <= from {
		return
	}
	type overlap struct {
	...
	for _, sg := range segs {
		...
		if hi > lo {
			ovs = append(ovs, overlap{sg.DeviceID, hi - lo})
			total += hi - lo
		}
	}
	if total == 0 {
