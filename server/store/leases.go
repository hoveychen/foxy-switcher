...
//
// Exclusivity is universal: every provider is single-holder, because two
// agents injecting the same credential fight over the same rate-limit window
// and session state. The whole thing runs inside a transaction so two agents
// racing to acquire the same account can't both succeed.
...
type AccountWithLease struct {
	Account
	// Lease is the account's single live holder. Nil when unheld — every
	// provider is exclusive, so there is never more than one.
	Lease *LeaseInfo
}
...
accounts.five_hour_util, accounts.five_hour_resets_at, accounts.five_hour_window_seconds,
accounts.seven_day_util, accounts.seven_day_resets_at, accounts.seven_day_window_seconds,
...
accounts.account_uuid, accounts.provider_user_id, accounts.rate_limit_tier, accounts.credential_json, accounts.codex_usage_json, accounts.pinned_device_id`
